...
-   **平台兼容性**: 许多多模态模型（包括 Gemini、OpenAI o系列等）无法直接读取 GIF。转换后的视频与预览帧可稳定地被这些模型处理。
-   **减少错误**: 避免因 GIF 格式不兼容导致的 LLM 请求失败或内容缺失。
...
| `preview_frame_count` | 生成多少帧 PNG 作为 GIF 预览。帧越多，LLM 越能理解动画，但请求体积也会增大。                                   | `4`         |
...
4.  **确认服务商**: 确认您使用的 LLM 服务商确实需要 GIF → 视频/图片（大部分多模态模型均适用）。
