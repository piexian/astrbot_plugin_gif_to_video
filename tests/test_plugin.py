...
class TestGifToVideoPlugin:
    ...
    def mock_context(self):
        ...
        context.get_using_provider = Mock(return_value=context.provider_manager.curr_provider_inst)
        return context
        ...
        with patch.object(plugin, 'default_provider_id', 'openai'):
            with patch('pathlib.Path.exists', return_value=True):
                with patch('shutil.copy2'):
                    with patch.object(plugin, '_get_default_provider_id', return_value='openai'):
                        result = await plugin.handle_gif_message(mock_event, mock_request)
